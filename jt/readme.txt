Тестовое задание выполненное по просьбе работодателя, с исправленными замечаниями

У пользователя есть phone, email, account. Написаны некоторые API для этого дела, потокобезопасный перевод денег.
Реальзован JWT token. Написаны тесты, в том числе используя MockMVC.
